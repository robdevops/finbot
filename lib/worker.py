...
def process_request(service, chat_id, user, message, botName, userRealName, message_id):
    ...
    rating_command = r"^([\!\.]\s?|^" + botName + r"\s+)(rate|rating)\s*([\w\s]+)*"
    m_rating = re.match(rating_command, message, re.IGNORECASE)
    ...
    if m_watchlist:
        ...
        if m_value.group(2) in allowed_actions:
            ...
            webhook.payload_wrapper(service, url, payload, chat_id)
            sys.exit(1)
        payload = reports.prepare_value_payload(service, action, length=15)
        if payload:
            webhook.payload_wrapper(service, url, payload, chat_id)
    elif m_beta:
        def last_col(e):
        ...
        if m_rating.group(3) in allowed_actions:
            action = m_rating.group(3)
        else:
            allowed_actions_str = '\n'.join(allowed_actions)
            payload = [f"valid 'rating' parameters:\n{allowed_actions_str}"]
            webhook.payload_wrapper(service, url, payload, chat_id)
            sys.exit(1)
        ...
        if payload:
            ...
            payload = [f"No stocks meet rating {action} criteria"]
            webhook.payload_wrapper(service, url, payload, chat_id)
    elif m_history:
        payload = []
        graph = None
